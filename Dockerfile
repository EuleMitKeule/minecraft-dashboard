FROM python:3.13-slim AS spec-generator
...
RUN uv run python -m minecraft_dashboard --generate-openapi openapi.json
...
COPY --from=spec-generator /app/openapi.json ../openapi.json
RUN npm run build
